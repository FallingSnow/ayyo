...
export class Router extends Middleware {
    ...
    async render({req, ...rest}) {
        if ((await super.render.apply(this, arguments)) === DONE) return DONE;
...
function paramsMiddleware(paramsParser) {
    return function paramsParserMiddleware({req}) {
...
export class Static extends Route {
    constructor({directory, path, method = "GET", ...rest}) {
        super({method, path: `${path}/**`, ...rest, handler: Static.handler});
