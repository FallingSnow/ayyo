...
export class Static extends Route {
  ...
  static async handler({
    ...
    let filepath = decodeURIComponent(req.url.relative.substring(this.originalPath.length));
    ...
    if (
      ...
      try {
        await this.fs.promises.access(webpPath);
        filepath = webpPath;
    ...
    try {
      if (req.headers[HTTP2_HEADER_RANGE]) {
        ...
        fileHandle = await this.fs.promises.open(systemPath);
        ...
        const allocationSize = end - start;
        const inputBuffer = Buffer.alloc(allocationSize);
        const {bytesRead, buffer} = await fileHandle.read(inputBuffer, 0, allocationSize, start);
        res.headers[HTTP2_HEADER_CONTENT_LENGTH] = bytesRead;
        ...
        const handle = await this.fs.promises.open(systemPath, 'r');
        res.file = handle.fd;
        // Close the file descriptor after the stream has ended
