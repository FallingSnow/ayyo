...
const METHODS = ["GET", "HEAD", "POST", "PUT", "DELETE", "CONNECT", "OPTIONS", "TRACE"];
...
export class Router extends Middleware {
    ...
    async render({
        ...
        for (const middleware of this.chain) {
            await middleware({req, searchPath, ...rest});
        }
        ...
        searchPath = (searchPath || removeParams(`${pathname}/${method}`)).substring(this.path.length);
    ...
    async use(...middlewares) {
        ...
        for (const middleware of middlewares) {
            ...
            assert(middleware instanceof Middleware, 'Cannot add non-middleware to router');

            if (middleware instanceof Route || middleware instanceof Router) {

                // Remove parameters for path and add target method
                const simplifiedPath = removeParams(`${middleware.path.replace(/\/$/, '')}/${middleware.method || '*'}`);
...
export class Route extends Middleware {
    constructor({
        ...
        assert(~[...METHODS, '*'].indexOf(method), `${method} is not a valid HTTP method`);
