...
export class OpenApi extends Router {
    ...
    static routeToOpenapi({method, path, data}, realPath) {
        const {operationId = pathToId(path, method), schema = {}, ...openapi} =
            data.openapi || {};
...
function joiParameterToOpenApiBody({
    ...
    if (type === "alternatives" && base)
        children = base.children;
    const properties = Object.keys(children).filter(name => {
        return get(children[name], "flags.presence") !== "forbidden";
    }).reduce((obj, name) => {
        obj[name] = joiParameterToOpenApiBody(children[name]);
        return obj;
    }, {});
    // console.log(alternatives)
    const {discriminator} = alternatives.reduce(({peek, then}, obj) => {
        if (!peek || !then)
            return obj;
        obj.discriminator = {propertyName: Object.keys(get(peek, "children"))[0]};
    ...
    return {
        ...
        items: Array.isArray(items)
            ? joiParameterToOpenApiBody(items[0])
            : undefined,
