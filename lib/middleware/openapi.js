...
export class OpenApi extends Router {
    constructor({
        ...
        this.options = {
            serveRedocPath,
            serveApiPath,
            validate: Object.assign({contentType: true, requests: true, responses: true}, validate)
        };
    ...
    async documentPaths(middleware, pathSegments = []) {
        ...
        if (middleware instanceof Route) {
            ...
            middleware.handler = async function handler({req, res}) {
                ...
                const contentTypes = get(data, "openapi.schema.consumes.contentTypes");
                if (options.validate.contentType && contentTypes && !~contentTypes.indexOf(req.headers[HTTP2_HEADER_CONTENT_TYPE]))
