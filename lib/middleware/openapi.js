...
export class OpenApi extends Router {
        ...
        if (this.options.serveRedocPath)
            await this.use(new Route({
                ...
                handler: ({
                    ...
                    res.body = `<style>html,body{margin:0;padding:0;}</style><redoc spec-url='.${this.opeions.serveApiPath}'></redoc><script src="https://cdn.jsdelivr.net/npm/redoc/bundles/redoc.standalone.js"></script>`;
                },
                openapi: {
                    tags: ['api'],
    ...
    async documentPaths(middleware, pathSegments = []) {
        ...
        if (middleware instanceof Route) {
            ...
            const documentedRoute = OpenApi.routeToOpenapi(middleware, realPath);
            set(this.doc, `paths.${realPath}.${method.toLowerCase()}`, documentedRoute);

            const originalHandler = handler;

            // Intercept the original handler to add validation
            middleware.handler = async function handler({
                req,
                res
            }) {
                await OpenApi.validate(get(data, 'openapi.schema.consumes'), req);
                const results = await originalHandler.apply(this, arguments);
                await OpenApi.validate(get(data, 'openapi.schema.produces'), res);
                return results;
            };
        }
        ...
        } else if (!(middleware instanceof Middleware) && typeof middleware === 'object') {
    ...
    static routeToOpenapi({
        path,
        method,
        data
    }) {
        const {
            operationId = pathToId(path),
                schema = {},
                ...openapi
        } = (data.openapi || {});
        ...
        if (pathParams) {
            ...
            parameters = [...parameters, ...Object.keys(structure).map(name => {
                return { ...joiParameterToOpenApi({ ...structure[name],
                        name
                    }),
                    in: 'path'
                };
        ...
        if (queryParams) {
            ...
            parameters = [...parameters, ...Object.keys(structure).map(name => {
                return {
                    ...joiParameterToOpenApi({ ...structure[name],
                        name
                    }),
                    in: 'query'
                };
    ...
    static async validate({
        ...
        const errors = [{
            ...
            Joi.validate(input, schema).error
        );
...
function joiParameterToOpenApiBody({
    ...
    return {
        ...
        required: Object.keys(children).filter(name => {
            return get(children[name], 'flags.presence') === 'required' ? true : false;
        })
    };
}
