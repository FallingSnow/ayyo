...
export class OpenApi extends Router {
        ...
        for (const [
            ...
            if (contentType && body)
                responses[code].content = {
                    [contentType]: {
                        schema: this.joiParameterToOpenApiBody({name: operationId, ...body.describe()})
                    }
                };
        }
    ...
    joiParameterToOpenApiBody({
        ...
        name = name || label || this.componentCounter++;
        ...
        let discriminator = alternatives.reduce((obj, {peek, then}) => {
            ...
            const oneOfSchema = this.joiParameterToOpenApiBody({name: peek.children[propertyName].valids[0], ...then});
